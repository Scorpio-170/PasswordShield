<?php
    ...
    if(isset($_POST['resetwithOTPbtn'])){
        ...
        if($resultCheck!=0)  
        {
          if($row= mysqli_fetch_assoc($result)){
            ...
            if(!$mail->send()){
                ...
                header("Location: chrome-extension://aecdplfdmmhbdjflimkoagbocjnaaifl/accountSettings/chgMstPassword/forgetMstPwOtpReceive.html");
            }
          }else{
            echo "something went wrong";
          }
        ...
        if($verifyOTP==$existingOTP){
            unset($_SESSION['OTP']);
            header("Location: chrome-extension://aecdplfdmmhbdjflimkoagbocjnaaifl/accountSettings/chgMstPassword/OTPResetMstPw.html");
        }else{
            header("Location: chrome-extension://aecdplfdmmhbdjflimkoagbocjnaaifl/accountSettings/chgMstPassword/forgetMstPwOtpReceiveErr.html");
        ...
        if($resultCheck!=0)  
        {
            if($row= mysqli_fetch_assoc($result)){
                ...
                if(password_verify($newPw, $oldPW)){
                    header("Location: chrome-extension://aecdplfdmmhbdjflimkoagbocjnaaifl/accountSettings/chgMstPassword/OTPResetMstPwErr.html");
                }else{
