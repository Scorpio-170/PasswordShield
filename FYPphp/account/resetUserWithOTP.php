<?php
    ...
    if(isset($_POST['resetwithOTPbtn'])){
        ...
        if($resultCheck!=0)  
        {
          if($row= mysqli_fetch_assoc($result)){
            ...
            if(!$mail->send()){
                ...
                header("Location: chrome-extension://kgnbabmgfmiogjhddmkkablkclfjdbcl/accountSettings/chgPassword/forgetPwOtpReceive.html");
            }
          }else{
            echo "something went wrong";
          }
        ...
        $verifyOTP = $con -> real_escape_string($_POST['verifyUserOTP']);
        $existingOTP = $_SESSION['OTP'];
        if($verifyOTP==$existingOTP){
            unset($_SESSION['OTP']);
            header("Location: chrome-extension://kgnbabmgfmiogjhddmkkablkclfjdbcl/accountSettings/chgPassword/OTPResetPw.html");
        }else{
            header("Location: chrome-extension://kgnbabmgfmiogjhddmkkablkclfjdbcl/accountSettings/chgPassword/forgetPwOtpReceiveErr.html");

        }
    }else if(isset($_POST['OTPResetUser'])){
        $userID = $_SESSION['sess_user'];
        ...
        if($resultCheck!=0)  
        {
            if($row= mysqli_fetch_assoc($result)){
                ...
                if(password_verify($newPw, $oldPW)){
                    header("Location: chrome-extension://kgnbabmgfmiogjhddmkkablkclfjdbcl/accountSettings/chgPassword/OTPResetPwErr.html");
                }else{
