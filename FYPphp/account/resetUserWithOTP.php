<?php
    ...
    if(isset($_POST['resetwithOTPbtn'])){
        ...
        if($resultCheck!=0)  
        {
          if($row= mysqli_fetch_assoc($result)){
            ...
            if(!$mail->send()){
                ...
                header("Location: chrome-extension://aecdplfdmmhbdjflimkoagbocjnaaifl/accountSettings/chgPassword/forgetPwOtpReceive.html");
            }
          }else{
            echo "something went wrong";
          }
        ...
        if($verifyOTP==$existingOTP){
            unset($_SESSION['OTP']);
            header("Location: chrome-extension://aecdplfdmmhbdjflimkoagbocjnaaifl/accountSettings/chgPassword/OTPResetPw.html");
        }else{
            header("Location: chrome-extension://aecdplfdmmhbdjflimkoagbocjnaaifl/accountSettings/chgPassword/forgetPwOtpReceiveErr.html");
        ...
        if($resultCheck!=0)  
        {
            if($row= mysqli_fetch_assoc($result)){
                ...
                if(password_verify($newPw, $oldPW)){
                    header("Location: chrome-extension://aecdplfdmmhbdjflimkoagbocjnaaifl/accountSettings/chgPassword/OTPResetPwErr.html");
                }else{
