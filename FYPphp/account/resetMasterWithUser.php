<?php
    $currentPw = $con -> real_escape_string($_POST['currentUserPassword']);
    $newPw = $con -> real_escape_string($_POST['newPassword']);
    $con = mysqli_connect('localhost','root','','fyp');
    session_start();
    $user_id = $_SESSION['sess_user'];
    if(isset($_POST['updatePwWithUsrBtn'])){
       ...
       if($resultCheck!=0)  
       {
         if($row= mysqli_fetch_assoc($result)){
             ...
             if(password_verify($currentPw, $userPW)){
                if(password_verify($newPw, $userMstPW)){
                    header("Location: chrome-extension://aecdplfdmmhbdjflimkoagbocjnaaifl/accountSettings/chgMstPassword/forgetPwUsrErr1.html");
                }else{
                 ...
                 header("Location: chrome-extension://aecdplfdmmhbdjflimkoagbocjnaaifl/accountSettings/chgMstPassword/forgetPwUsrErr.html");
             }
        }else{
        echo "something went wrong";
             }
